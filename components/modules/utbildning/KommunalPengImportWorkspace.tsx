...
const EXPECTED_COLUMNS = ['Period', 'Antal barn', 'Peng per barn', 'Totalt belopp', 'Kontonummer']
...
export function KommunalPengImportWorkspace({ module, sectorSlug, settingsHref }: ModuleWorkspaceProps) {
  ...
  const loadImports = useCallback(async () => {
    ...
    const { data } = await supabase.from('module_imports').select('id, filename, created_at, rows_imported, status')
      .eq('user_id', user.id).eq('sector_slug', sectorSlug).eq('module_slug', 'kommunal-peng-import').order('created_at', { ascending: false })
    setImports(data ?? [])
  ...
  async function handleConfirmImport() {
    ...
    await supabase.from('module_imports').insert({
      user_id: user.id, sector_slug: sectorSlug, module_slug: 'kommunal-peng-import',
  ...
  return (
    <ModuleWorkspaceShell title={module.name} description={module.desc} category={module.cat as 'import'} sectorName="Utbildning & Förskola" backHref={`/m/${sectorSlug}`} settingsHref={settingsHref}
      tabs={
        <Tabs defaultValue="importera" className="w-full">
          ...
          <TabsContent value="importera" className="space-y-6 mt-6">
            {!parsedData && !saved && (
              <ImportDropzone accept=".csv,.xlsx,.xls" onFileSelect={handleFileSelect} label="Dra och släpp kommunal peng-fil här" description="CSV eller Excel med kolumner: Period, Antal barn, Peng per barn, Totalt belopp, Kontonummer" />
            )}
            {saved && (
          ...
          <TabsContent value="historik" className="mt-6">
            {loadingImports ? (
              ...
              <EmptyModuleState icon={History} title="Ingen importhistorik" description="När du importerar kommunal peng-data visas den här." />
            ) : (
