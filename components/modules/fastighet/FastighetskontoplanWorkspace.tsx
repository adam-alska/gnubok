...
type AccountCategory = 'Byggnader' | 'Hyresintäkter' | 'Drift' | 'Underhåll' | 'Skatt' | 'Avskrivning' | 'Finansiellt' | 'Övrigt'
...
const ACCOUNT_CATEGORIES: AccountCategory[] = ['Byggnader', 'Hyresintäkter', 'Drift', 'Underhåll', 'Skatt', 'Avskrivning', 'Finansiellt', 'Övrigt']
const DEFAULT_ACCOUNTS: Account[] = [
  { number: '1110', name: 'Byggnader', type: 'Tillgång', category: 'Byggnader' },
  { number: '1119', name: 'Ack. avskrivning byggnader', type: 'Tillgång', category: 'Avskrivning' },
  { number: '1150', name: 'Markanläggningar', type: 'Tillgång', category: 'Byggnader' },
  { number: '3010', name: 'Hyresintäkter bostäder', type: 'Intäkt', category: 'Hyresintäkter' },
  { number: '3020', name: 'Hyresintäkter lokaler', type: 'Intäkt', category: 'Hyresintäkter' },
  { number: '3030', name: 'Hyresintäkter parkering', type: 'Intäkt', category: 'Hyresintäkter' },
  { number: '5010', name: 'Fastighetsskötsel', type: 'Kostnad', category: 'Drift' },
  { number: '5020', name: 'Reparation och underhåll', type: 'Kostnad', category: 'Underhåll' },
  { number: '5030', name: 'Vatten och avlopp', type: 'Kostnad', category: 'Drift' },
  { number: '5040', name: 'Uppvärmning', type: 'Kostnad', category: 'Drift' },
  { number: '5050', name: 'El', type: 'Kostnad', category: 'Drift' },
  { number: '5060', name: 'Sophämtning', type: 'Kostnad', category: 'Drift' },
  { number: '7720', name: 'Fastighetsskatt', type: 'Kostnad', category: 'Skatt' },
  { number: '7820', name: 'Avskrivning byggnader', type: 'Kostnad', category: 'Avskrivning' },
  { number: '2970', name: 'Förutbetalda hyresintäkter', type: 'Skuld', category: 'Hyresintäkter' },
]
...
export function FastighetskontoplanWorkspace({ module: mod, sectorSlug, settingsHref }: ModuleWorkspaceProps) {
  ...
  return (
    <>
      <ModuleWorkspaceShell title={mod.name} description={mod.desc} category="bokforing" sectorName="Fastighet" backHref={`/m/${sectorSlug}`} settingsHref={settingsHref} actions={<Button onClick={openNew}><Plus className="mr-2 h-4 w-4" />Nytt konto</Button>}>
        {loading ? <div className="flex items-center justify-center py-12"><Loader2 className="h-6 w-6 animate-spin text-muted-foreground" /></div> : (
          <div className="space-y-6">
            ...
            {filteredAccounts.length === 0 ? <EmptyModuleState icon={FileSpreadsheet} title="Inga konton hittades" description="Lägg till konton för fastighetsförvaltning." actionLabel="Nytt konto" onAction={openNew} /> : (
