...
interface EnergyRow { property: string; period: string; electricityKwh: number; heatingKwh: number; waterM3: number; totalCost: number; costPerM2: number }
...
export function EnergirapportImportWorkspace({ module: mod, sectorSlug, settingsHref }: ModuleWorkspaceProps) {
  const supabase = createClient()
  const [loading, setLoading] = useState(true); const [preview, setPreview] = useState<EnergyRow[]>([]); const [fileName, setFileName] = useState(''); const [history, setHistory] = useState<ImportRecord[]>([]); const [importing, setImporting] = useState(false)
  ...
  function handleFile(file: File) {
    ...
    reader.onload = (ev) => {
      ...
      for (let i = 1; i < lines.length; i++) { const cols = lines[i].split(sep).map(c => c.trim().replace(/^"|"$/g, '')); if (cols.length >= 5) { rows.push({ property: cols[0], period: cols[1], electricityKwh: parseFloat(cols[2]) || 0, heatingKwh: parseFloat(cols[3]) || 0, waterM3: parseFloat(cols[4]) || 0, totalCost: parseFloat(cols[5]) || 0, costPerM2: parseFloat(cols[6]) || 0 }) } }
      setPreview(rows)
  ...
  async function confirmImport() {
    ...
    await supabase.from('module_configs').upsert({ user_id: user.id, sector_slug: sectorSlug, module_slug: mod.slug, config_key: 'energy_data', config_value: preview }, { onConflict: 'user_id,sector_slug,module_slug,config_key' })
    ...
    <ModuleWorkspaceShell title={mod.name} description={mod.desc} category="import" sectorName="Fastighet" backHref={`/m/${sectorSlug}`} settingsHref={settingsHref}>
      <Tabs defaultValue="import" className="space-y-6">
        ...
        <TabsContent value="import" className="space-y-6">
          <ImportDropzone accept=".csv" onFileSelect={handleFile} label="Dra och släpp energirapport" description="CSV med kolumner: Fastighet, Period, El kWh, Värme kWh, Vatten m³, Totalkostnad, kr/m²" />
          {preview.length > 0 && (
            <div className="space-y-4">
              ...
              <div className="rounded-xl border border-border overflow-hidden max-h-96 overflow-y-auto"><table className="w-full text-sm"><thead><tr className="bg-muted/50 border-b sticky top-0"><th className="text-left px-4 py-3 font-medium text-muted-foreground">Fastighet</th><th className="text-left px-4 py-3 font-medium text-muted-foreground">Period</th><th className="text-right px-4 py-3 font-medium text-muted-foreground">El kWh</th><th className="text-right px-4 py-3 font-medium text-muted-foreground">Värme kWh</th><th className="text-right px-4 py-3 font-medium text-muted-foreground">Vatten m³</th><th className="text-right px-4 py-3 font-medium text-muted-foreground">Kostnad</th></tr></thead><tbody>{preview.slice(0, 50).map((r, i) => <tr key={i} className="border-b last:border-0"><td className="px-4 py-2">{r.property}</td><td className="px-4 py-2">{r.period}</td><td className="px-4 py-2 text-right tabular-nums">{fmt(r.electricityKwh)}</td><td className="px-4 py-2 text-right tabular-nums">{fmt(r.heatingKwh)}</td><td className="px-4 py-2 text-right tabular-nums">{fmt(r.waterM3)}</td><td className="px-4 py-2 text-right tabular-nums font-medium">{fmt(r.totalCost)}</td></tr>)}</tbody></table></div>
            </div>
          )}
        </TabsContent>
        <TabsContent value="historik" className="space-y-4">
          {loading ? <div className="flex items-center justify-center py-12"><Loader2 className="h-6 w-6 animate-spin text-muted-foreground" /></div> : history.length === 0 ? <EmptyModuleState icon={Upload} title="Ingen importhistorik" description="Importera energirapporter via CSV." /> : (
