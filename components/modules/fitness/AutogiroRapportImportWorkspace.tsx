...
const EXPECTED_COLUMNS = ['Betalarnummer', 'Namn', 'Belopp', 'Bankgiro', 'Status', 'Datum']
...
export function AutogiroRapportImportWorkspace({ module, sectorSlug, settingsHref }: ModuleWorkspaceProps) {
  ...
  const loadImports = useCallback(async () => {
    ...
    const { data } = await supabase.from('module_imports').select('id, filename, created_at, rows_imported, status').eq('user_id', user.id).eq('sector_slug', sectorSlug).eq('module_slug', 'autogiro-rapport-import').order('created_at', { ascending: false })
    setImports(data ?? [])
  ...
  async function handleConfirmImport() {
    ...
    await supabase.from('module_imports').insert({ user_id: user.id, sector_slug: sectorSlug, module_slug: 'autogiro-rapport-import', filename, status: 'completed', rows_imported: parsedData.rows.length, import_data: importData })
      ...
      tabs={
        <Tabs defaultValue="importera" className="w-full">
          ...
          <TabsContent value="importera" className="space-y-6 mt-6">
            {!parsedData && !saved && <ImportDropzone accept=".csv,.xlsx,.xls" onFileSelect={handleFileSelect} label="Dra och släpp din Bankgirot-rapport här" description="CSV eller Excel med kolumner: Betalarnummer, Namn, Belopp, Bankgiro, Status, Datum" />}
            {saved && (
            ...
            {parsedData && !saved && (
              <div className="space-y-4">
                ...
                <div className="rounded-lg border overflow-auto max-h-96">
                  <table className="w-full text-sm"><thead><tr className="border-b bg-muted/50">{parsedData.headers.map((header, i) => (<th key={i} className="px-3 py-2 text-left font-medium text-muted-foreground whitespace-nowrap">{header}</th>))}</tr></thead><tbody>{parsedData.rows.slice(0, 20).map((row, ri) => (<tr key={ri} className="border-b last:border-0">{row.map((cell, ci) => (<td key={ci} className="px-3 py-2 whitespace-nowrap">{cell}</td>))}</tr>))}</tbody></table>
          ...
          <TabsContent value="historik" className="mt-6">
            {loadingImports ? (
              ...
              <EmptyModuleState icon={History} title="Ingen importhistorik" description="När du importerar autogiro-rapporter visas de här." />
            ) : (
