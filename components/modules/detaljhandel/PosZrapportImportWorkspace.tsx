...
export function PosZrapportImportWorkspace({ module, sectorSlug, settingsHref }: ModuleWorkspaceProps) {
  ...
  const loadImports = useCallback(async () => {
    ...
    const { data } = await supabase
      ...
      .eq('module_slug', 'pos-z-rapport-import')
  ...
  async function handleConfirmImport() {
    ...
    await supabase.from('module_imports').insert({
      ...
      module_slug: 'pos-z-rapport-import',
      filename,
      status: 'completed',
              ...
              <div className="space-y-4">
                ...
                <div className="rounded-lg border overflow-auto max-h-96">
                    ...
                    <thead>
                      <tr className="border-b bg-muted/50">
                        {parsedData.headers.map((header, i) => (
                          <th key={i} className="px-3 py-2 text-left font-medium text-muted-foreground whitespace-nowrap">{header}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                  ...
                  {parsedData.rows.length > 20 && (
                    <p className="px-3 py-2 text-xs text-muted-foreground bg-muted/30">Visar 20 av {parsedData.rows.length} rader...</p>
                  )}
                </div>
