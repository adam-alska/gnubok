...
interface TicketRow { event: string; ticketType: string; quantity: number; unitPrice: number; totalAmount: number; saleDate: string }
...
export function BiljettsystemImportWorkspace({ module: mod, sectorSlug, settingsHref }: ModuleWorkspaceProps) {
  const supabase = createClient()
  const [loading, setLoading] = useState(true); const [preview, setPreview] = useState<TicketRow[]>([]); const [fileName, setFileName] = useState(''); const [history, setHistory] = useState<ImportRecord[]>([]); const [importing, setImporting] = useState(false)
  ...
  function handleFile(file: File) {
    ...
    reader.onload = (ev) => {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        if (cols.length >= 5) {
          rows.push({ event: cols[0], ticketType: cols[1], quantity: parseInt(cols[2]) || 0, unitPrice: parseFloat(cols[3]) || 0, totalAmount: parseFloat(cols[4]) || 0, saleDate: cols[5] || '' })
        }
      }
      setPreview(rows)
    }
  ...
  async function confirmImport() {
    ...
    await supabase.from('module_configs').upsert({ user_id: user.id, sector_slug: sectorSlug, module_slug: mod.slug, config_key: 'ticket_data', config_value: preview }, { onConflict: 'user_id,sector_slug,module_slug,config_key' })
    ...
    <ModuleWorkspaceShell title={mod.name} description={mod.desc} category="import" sectorName="Event" backHref={`/m/${sectorSlug}`} settingsHref={settingsHref}>
      <Tabs defaultValue="import" className="space-y-6">
        ...
        <TabsContent value="import" className="space-y-6">
          <ImportDropzone accept=".csv" onFileSelect={handleFile} label="Dra och släpp biljettsystem-export" description="CSV med kolumner: Event, Biljetttyp, Antal, Á-pris, Totalt, Datum" />
          {preview.length > 0 && (
            <div className="space-y-4">
              ...
              <div className="rounded-xl border border-border overflow-hidden max-h-96 overflow-y-auto"><table className="w-full text-sm"><thead><tr className="bg-muted/50 border-b sticky top-0"><th className="text-left px-4 py-3 font-medium text-muted-foreground">Event</th><th className="text-left px-4 py-3 font-medium text-muted-foreground">Biljetttyp</th><th className="text-right px-4 py-3 font-medium text-muted-foreground">Antal</th><th className="text-right px-4 py-3 font-medium text-muted-foreground">Á-pris</th><th className="text-right px-4 py-3 font-medium text-muted-foreground">Totalt</th><th className="text-left px-4 py-3 font-medium text-muted-foreground">Datum</th></tr></thead><tbody>{preview.slice(0, 50).map((r, i) => <tr key={i} className="border-b last:border-0"><td className="px-4 py-2">{r.event}</td><td className="px-4 py-2">{r.ticketType}</td><td className="px-4 py-2 text-right tabular-nums">{r.quantity}</td><td className="px-4 py-2 text-right tabular-nums">{fmt(r.unitPrice)}</td><td className="px-4 py-2 text-right tabular-nums font-medium">{fmt(r.totalAmount)}</td><td className="px-4 py-2">{r.saleDate}</td></tr>)}</tbody></table></div>
        ...
        <TabsContent value="historik" className="space-y-4">
          {loading ? <div className="flex items-center justify-center py-12"><Loader2 className="h-6 w-6 animate-spin text-muted-foreground" /></div> : history.length === 0 ? <EmptyModuleState icon={Upload} title="Ingen importhistorik" description="Importera biljettsystem-data via CSV-fil." /> : (
