...
const EXPECTED_COLUMNS = ['Artikelnr', 'Artikelnamn', 'Lagernivå', 'Räknat antal', 'Systemantal', 'Differens', 'Datum']
...
export function LagerexportImportWorkspace({ module, sectorSlug, settingsHref }: ModuleWorkspaceProps) {
  ...
  const loadImports = useCallback(async () => {
    ...
    const { data } = await supabase.from('module_configs').select('config_value')
      .eq('user_id', user.id).eq('sector_slug', sectorSlug).eq('module_slug', module.slug)
      .eq('config_key', 'import_history').maybeSingle()
    if (data?.config_value && Array.isArray(data.config_value)) setImports(data.config_value as ImportRecord[])
    setLoadingImports(false)
  }, [supabase, sectorSlug, module.slug])
  ...
  async function handleConfirmImport() {
    ...
    await supabase.from('module_configs').upsert(
      { user_id: user.id, sector_slug: sectorSlug, module_slug: module.slug, config_key: 'import_history', config_value: updatedImports },
  ...
  return (
    <ModuleWorkspaceShell
      title={module.name} description={module.desc} category={module.cat as 'import'}
      sectorName="Tillverkning" backHref={`/m/${sectorSlug}`} settingsHref={settingsHref}
      tabs={
        <Tabs defaultValue="importera" className="w-full">
          ...
          <TabsContent value="importera" className="space-y-6 mt-6">
            {!parsedData && !saved && <ImportDropzone accept=".csv,.xlsx,.xls" onFileSelect={handleFileSelect} label="Dra och släpp lagerinventeringsfil här" description={`CSV eller Excel med kolumner: ${EXPECTED_COLUMNS.join(', ')}`} />}
            {saved && (
              <div className="flex flex-col items-center gap-3 rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-8">
                <CheckCircle className="h-8 w-8 text-emerald-600" />
                <p className="text-sm font-medium text-emerald-700">Importen är klar!</p>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button size="sm" onClick={handleConfirmImport} disabled={saving}>{saving && <Loader2 className="mr-2 h-3.5 w-3.5 animate-spin" />}Bekräfta import</Button>
                  </div>
                </div>
                <div className="rounded-lg border overflow-auto max-h-96">
                  <table className="w-full text-sm">
                    ...
                    <tbody>{parsedData.rows.slice(0, 20).map((row, ri) => <tr key={ri} className="border-b last:border-0">{row.map((cell, ci) => <td key={ci} className="px-3 py-2 whitespace-nowrap">{cell}</td>)}</tr>)}</tbody>
                  </table>
          ...
          <TabsContent value="historik" className="mt-6">
            {loadingImports ? (
              ...
              <EmptyModuleState icon={History} title="Ingen importhistorik" description="När du importerar lagerdata visas de här." />
            ) : (
