...
const EXPECTED_COLUMNS = ['Datum', 'Produkt', 'Godkända', 'Kasserade', 'Materialförbrukning (kg)', 'Maskin']
...
export function ProduktionsrapportImportWorkspace({ module, sectorSlug, settingsHref }: ModuleWorkspaceProps) {
  ...
  function handleFileSelect(file: File) {
    ...
    reader.onload = (e) => {
      ...
      if (text) setParsedData(parseCSV(text))
    }
  ...
  async function handleConfirmImport() {
    ...
    await supabase.from('module_configs').upsert(
      { user_id: user.id, sector_slug: sectorSlug, module_slug: module.slug, config_key: `import_data_${newImport.id}`, config_value: importData },
      { onConflict: 'user_id,sector_slug,module_slug,config_key' }
    )
      ...
      tabs={
        <Tabs defaultValue="importera" className="w-full">
          ...
          <TabsContent value="importera" className="space-y-6 mt-6">
            {!parsedData && !saved && (
              <ImportDropzone accept=".csv,.xlsx,.xls" onFileSelect={handleFileSelect}
                label="Dra och släpp din produktionsrapport här"
                description={`CSV eller Excel med kolumner: ${EXPECTED_COLUMNS.join(', ')}`}
              />
            )}
            {saved && (
          ...
          <TabsContent value="historik" className="mt-6">
            {loadingImports ? (
              ...
              <EmptyModuleState icon={History} title="Ingen importhistorik" description="När du importerar produktionsrapporter visas de här." />
            ) : (
