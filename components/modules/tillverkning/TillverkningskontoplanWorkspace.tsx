...
type AccountCategory = 'Råmaterial' | 'PIA' | 'Färdigvaror' | 'Inköp' | 'Maskiner' | 'Energi' | 'Personal' | 'Övrigt'
...
const ACCOUNT_CATEGORIES: AccountCategory[] = ['Råmaterial', 'PIA', 'Färdigvaror', 'Inköp', 'Maskiner', 'Energi', 'Personal', 'Övrigt']

const DEFAULT_ACCOUNTS: Account[] = [
  { number: '1410', name: 'Lager råmaterial', type: 'Tillgång', category: 'Råmaterial' },
  { number: '1420', name: 'Produkter i arbete (PIA)', type: 'Tillgång', category: 'PIA' },
  { number: '1450', name: 'Lager färdigvaror', type: 'Tillgång', category: 'Färdigvaror' },
  { number: '1210', name: 'Maskiner och inventarier', type: 'Tillgång', category: 'Maskiner' },
  { number: '3010', name: 'Försäljning tillverkade produkter', type: 'Intäkt', category: 'Färdigvaror' },
  { number: '4010', name: 'Inköp råmaterial', type: 'Kostnad', category: 'Inköp' },
  { number: '4020', name: 'Inköp insatsvaror', type: 'Kostnad', category: 'Inköp' },
  { number: '4900', name: 'Produktionsavvikelser', type: 'Kostnad', category: 'Övrigt' },
  { number: '5010', name: 'Lokalkostnader produktion', type: 'Kostnad', category: 'Övrigt' },
  { number: '5020', name: 'Energikostnader', type: 'Kostnad', category: 'Energi' },
  { number: '7010', name: 'Löner produktionspersonal', type: 'Kostnad', category: 'Personal' },
  { number: '7510', name: 'Sociala avgifter', type: 'Kostnad', category: 'Personal' },
  { number: '7831', name: 'Avskrivning maskiner', type: 'Kostnad', category: 'Maskiner' },
]
...
export function TillverkningskontoplanWorkspace({ module: mod, sectorSlug, settingsHref }: ModuleWorkspaceProps) {
              ...
              <EmptyModuleState
                ...
                description={
                  searchQuery || filterType !== 'all'
                    ...
                    : 'Lägg till konton för att bygga upp din tillverkningskontoplan.'
                }
                      ...
                      <TableRow key={account.number}>
                        ...
                        <TableCell className="text-right">
                          <div className="flex items-center justify-end gap-1">
                            <Button variant="ghost" size="icon" onClick={() => openEditAccount(account)} title="Redigera">
                              <Pencil className="h-4 w-4" />
                            </Button>
                            <Button variant="ghost" size="icon" className="text-red-600 hover:text-red-700" onClick={() => openDeleteConfirmation(account)} title="Ta bort">
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-md">
          ...
          <div className="grid gap-4 py-2">
            <div className="grid grid-cols-2 gap-4">
              <div className="grid gap-2">
                ...
                <Input id="acct-number" value={accountForm.number} onChange={(e) => setAccountForm((f) => ({ ...f, number: e.target.value }))} placeholder="1410" maxLength={6} disabled={!!editingAccount} />
              </div>
              <div className="grid gap-2">
                <Label htmlFor="acct-name">Kontonamn *</Label>
                <Input id="acct-name" value={accountForm.name} onChange={(e) => setAccountForm((f) => ({ ...f, name: e.target.value }))} placeholder="Lager råmaterial" />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div className="grid gap-2">
                <Label htmlFor="acct-type">Kontotyp *</Label>
                <Select value={accountForm.type} onValueChange={(val) => setAccountForm((f) => ({ ...f, type: val as AccountType }))}>
                  <SelectTrigger id="acct-type"><SelectValue placeholder="Välj typ" /></SelectTrigger>
                  <SelectContent>
              ...
              <div className="grid gap-2">
                ...
                <Select value={accountForm.category} onValueChange={(val) => setAccountForm((f) => ({ ...f, category: val as AccountCategory }))}>
                  <SelectTrigger id="acct-category"><SelectValue placeholder="Välj kategori" /></SelectTrigger>
                  <SelectContent>
                    {ACCOUNT_CATEGORIES.map((c) => (<SelectItem key={c} value={c}>{c}</SelectItem>))}
                  </SelectContent>
                </Select>
              </div>
            </div>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSaveAccount} disabled={!accountForm.number.trim() || !accountForm.name.trim()}>
