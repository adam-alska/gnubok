...
type AccountCategory = 'Projektarvoden' | 'Licensintäkter' | 'Royalties' | 'Freelancer' | 'Produktion' | 'IP-tillgångar' | 'Personal' | 'Övrigt'
...
const ACCOUNT_CATEGORIES: AccountCategory[] = ['Projektarvoden', 'Licensintäkter', 'Royalties', 'Freelancer', 'Produktion', 'IP-tillgångar', 'Personal', 'Övrigt']

const DEFAULT_ACCOUNTS: Account[] = [
  { number: '3010', name: 'Projektarvoden', type: 'Intäkt', category: 'Projektarvoden' },
  { number: '3020', name: 'Licensintäkter', type: 'Intäkt', category: 'Licensintäkter' },
  { number: '3910', name: 'Royalties', type: 'Intäkt', category: 'Royalties' },
  { number: '1010', name: 'Immateriella rättigheter', type: 'Tillgång', category: 'IP-tillgångar' },
  { number: '1020', name: 'Varumärken', type: 'Tillgång', category: 'IP-tillgångar' },
  { number: '4010', name: 'Freelancerkostnader', type: 'Kostnad', category: 'Freelancer' },
  { number: '4110', name: 'Produktionskostnader', type: 'Kostnad', category: 'Produktion' },
...
export function MediakontoplanWorkspace({ module: mod, sectorSlug, settingsHref }: ModuleWorkspaceProps) {
  ...
  const saveAccounts = useCallback(async (items: Account[]) => {
    ...
    await supabase.from('module_configs').upsert({ user_id: user.id, sector_slug: sectorSlug, module_slug: mod.slug, config_key: 'accounts', config_value: items }, { onConflict: 'user_id,sector_slug,module_slug,config_key' }); setSaving(false)
  ...
  const fetchAccounts = useCallback(async () => {
    ...
    if (data?.config_value && Array.isArray(data.config_value) && data.config_value.length > 0) { setAccounts(data.config_value as Account[]) } else {
  ...
  async function handleSave() { const entry: Account = { number: form.number.trim(), name: form.name.trim(), type: form.type, category: form.category }; let updated: Account[]; if (editing) { updated = accounts.map(a => a.number === editing.number ? entry : a) } else { if (accounts.some(a => a.number === entry.number)) return; updated = [...accounts, entry] }; setAccounts(updated); setDialogOpen(false); await saveAccounts(updated) }
  ...
  return (
    <>
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}><DialogContent className="max-w-sm"><DialogHeader><DialogTitle>Ta bort konto</DialogTitle><DialogDescription>Ta bort {toDelete?.number} ({toDelete?.name})?</DialogDescription></DialogHeader><DialogFooter><Button variant="outline" onClick={() => setDeleteDialogOpen(false)}>Avbryt</Button><Button variant="destructive" onClick={handleDelete}><Trash2 className="mr-2 h-4 w-4" />Ta bort</Button></DialogFooter></DialogContent></Dialog>
    </>
  )
}
