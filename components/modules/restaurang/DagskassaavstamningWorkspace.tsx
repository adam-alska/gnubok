...
export function DagskassaavstamningWorkspace({ module: mod, sectorSlug, settingsHref }: ModuleWorkspaceProps) {
  ...
  const monthAvgDiff = monthHistory.length > 0
    ...
    : 0
  const monthCount = monthHistory.length
  ...
  const fetchHistory = useCallback(async (dates: string[]) => {
    ...
    const configKeys = dates.map((d) => `dagskassa_${d}`)
  ...
  async function handleSave() {
    ...
    await supabase
      ...
      .upsert({
        ...
        config_key: `dagskassa_${formDate}`,
        config_value: reconciliation,
    ...
    await supabase
      ...
      .upsert({
        ...
        sector_slug: sectorSlug,
        module_slug: mod.slug,
        config_key: 'dagskassa_dates',
        config_value: updatedDates,
      }, { onConflict: 'user_id,sector_slug,module_slug,config_key' })
  ...
  return (
    ...
    >
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
        <TabsList>
          <TabsTrigger value="ny">Ny avst\u00e4mning</TabsTrigger>
        ...
        <TabsContent value="ny" className="space-y-6">
              ...
              <div className="grid gap-3">
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    onChange={(e) => setZReport((z) => ({ ...z, totalSales: numVal(e.target.value) }))}
                    placeholder="0"
                    className="h-9"
                  />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    ...
                    <Input
                      type="number"
                      step="0.01"
                      value={zReport.cardPayments || ''}
                      onChange={(e) => setZReport((z) => ({ ...z, cardPayments: numVal(e.target.value) }))}
                      placeholder="0"
                      className="h-9"
                    />
                  </div>
                  <div className="space-y-1.5">
                    ...
                    <Input
                      type="number"
                      step="0.01"
                      value={zReport.cashPayments || ''}
                      onChange={(e) => setZReport((z) => ({ ...z, cashPayments: numVal(e.target.value) }))}
                      placeholder="0"
                      className="h-9"
                    />
                  </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    ...
                    <Input
                      ...
                      step="0.01"
                      value={zReport.swish || ''}
                      onChange={(e) => setZReport((z) => ({ ...z, swish: numVal(e.target.value) }))}
                      placeholder="0"
                      className="h-9"
                    />
                  </div>
                  <div className="space-y-1.5">
                    ...
                    <Input
                      ...
                      step="0.01"
                      value={zReport.other || ''}
                      onChange={(e) => setZReport((z) => ({ ...z, other: numVal(e.target.value) }))}
                      placeholder="0"
                      className="h-9"
                    />
                  </div>
                ...
                <div className="grid gap-3">
                  <div className="space-y-1.5">
                    ...
                    <Input
                      type="number"
                      step="0.01"
                      value={actual.countedCash || ''}
                      onChange={(e) => setActual((a) => ({ ...a, countedCash: numVal(e.target.value) }))}
                      placeholder="0"
                      className="h-9"
                    />
                  </div>
                  <div className="space-y-1.5">
                    ...
                    <Input
                      type="number"
                      step="0.01"
                      value={actual.cashDeposits || ''}
                      onChange={(e) => setActual((a) => ({ ...a, cashDeposits: numVal(e.target.value) }))}
                      placeholder="0"
                      className="h-9"
                    />
                  </div>
              ...
              <div className="rounded-xl border border-border bg-card p-5 space-y-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <Input
                    type="number"
                    step="0.01"
                    value={bank.cardTerminalAmount || ''}
                    onChange={(e) => setBank((b) => ({ ...b, cardTerminalAmount: numVal(e.target.value) }))}
                    placeholder="0"
                    className="h-9"
                  />
                </div>
          ...
          <div className={cn(
            ...
            getDifferenceBg(totalDifference),
          )}>
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              <div className="space-y-1">
                ...
                <p className="text-xs text-muted-foreground">Z-kontant ({fmt(zReport.cashPayments)}) - Ins\u00e4ttningar ({fmt(actual.cashDeposits)})</p>
              </div>
              <div className="space-y-1">
                ...
                <p className="text-xs text-muted-foreground">R\u00e4knad ({fmt(actual.countedCash)}) - F\u00f6rv\u00e4ntad ({fmt(expectedCash)})</p>
              </div>
              <div className="space-y-1">
                ...
                <p className="text-xs text-muted-foreground">Bank ({fmt(bank.cardTerminalAmount)}) - Z-kort ({fmt(zReport.cardPayments)})</p>
              </div>
              <div className="space-y-1">
                ...
                <p className="text-xs text-muted-foreground">
                  {Math.abs(totalDifference) === 0 ? 'Ingen avvikelse' : Math.abs(totalDifference) < 50 ? 'Liten avvikelse' : 'Stor avvikelse'}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <Textarea
              ...
              placeholder="T.ex. anledning till differens, speciella h\u00e4ndelser under dagen..."
              rows={3}
            />
          </div>
          ...
          <div className="flex justify-end">
            <Button
              onClick={handleSave}
              disabled={saving || !formDate || zReport.totalSales === 0}
            >
        ...
        <TabsContent value="historik" className="space-y-4">
          ...
          {loading ? (
            ...
            <EmptyModuleState
              icon={Calculator}
              title="Inga avst\u00e4mningar"
              description="Det finns inga kassaavst\u00e4mningar f\u00f6r den valda m\u00e5naden. G\u00e5 till fliken Ny avst\u00e4mning f\u00f6r att skapa en."
                    ...
                    <>
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-center">
                          <StatusBadge
                            label={r.status === 'ok' ? 'OK' : 'Avvikelse'}
                            variant={r.status === 'ok' ? 'success' : 'danger'}
                          />
                        </td>
                            ...
                            <div className="grid gap-4 sm:grid-cols-3 text-xs">
                              <div className="space-y-2">
                                ...
                                <div className="space-y-1">
                                  ...
                                  <div className="flex justify-between"><span className="text-muted-foreground">\u00d6vrigt:</span><span className="tabular-nums">{fmt(r.zReport.other)} kr</span></div>
                                </div>
                              </div>
                              <div className="space-y-2">
                                ...
                                <div className="space-y-1">
                                  ...
                                  <div className="flex justify-between"><span className="text-muted-foreground">Kortterminal (bank):</span><span className="tabular-nums">{fmt(r.bank.cardTerminalAmount)} kr</span></div>
                                </div>
                              </div>
                              <div className="space-y-2">
                                <p className="font-semibold text-sm">Differenser</p>
                                <div className="space-y-1">
                                  <div className="flex justify-between"><span className="text-muted-foreground">F\u00f6rv\u00e4ntad kassa:</span><span className="tabular-nums">{fmtDecimal(r.differences.expectedCash)} kr</span></div>
                                  <div className="flex justify-between">
                                    <span className="text-muted-foreground">Kassadiff:</span>
                                    <span className={cn('tabular-nums', getDifferenceColor(r.differences.cashDifference))}>{r.differences.cashDifference >= 0 ? '+' : ''}{fmtDecimal(r.differences.cashDifference)} kr</span>
                                  </div>
                                  <div className="flex justify-between">
                                    <span className="text-muted-foreground">Kortdiff:</span>
                                    <span className={cn('tabular-nums', getDifferenceColor(r.differences.cardDifference))}>{r.differences.cardDifference >= 0 ? '+' : ''}{fmtDecimal(r.differences.cardDifference)} kr</span>
                                  </div>
                                  <div className="flex justify-between font-semibold">
                                    <span>Totaldiff:</span>
                                    <span className={cn('tabular-nums', getDifferenceColor(r.differences.totalDifference))}>{r.differences.totalDifference >= 0 ? '+' : ''}{fmtDecimal(r.differences.totalDifference)} kr</span>
                                  </div>
                                </div>
                                {r.notes && (
