...
function detectVatTreatment(result: MappingResult): string | null {
  ...
  // Determine rate from debit amount ratio
  // The input VAT line was generated from totalAmount, so we can't reliably
  // reverse the rate. Use the VAT account to infer.
  // For now, default to standard_25 if input VAT exists
  return 'standard_25'
}
